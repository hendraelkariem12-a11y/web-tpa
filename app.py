...
def get_minggu_sekarang():
    ...
    return dates_of_week, now.strftime('%Y-%m-%d'), now.year, now.strftime('%A')
...
@app.route('/')
def portal_orangtua():
    dates_week, today_db, current_year, day_name = get_minggu_sekarang()
        ...
        <div class="container py-3" style="max-width: 600px;">
            ...
            <div class="card border-0 bg-warning bg-opacity-10 border-warning shadow-sm p-3 mb-4 rounded-3 text-center">
                ...
                <p class="small text-muted mb-3">
                    Kami juga menyediakan warung dengan berbagai menu lezat. Sekalian Ayah/Bunda belanja, sama saja dengan <strong>beramal di Yayasan Amal Anak Nagari</strong> untuk mendukung pendidikan anak-anak.
                </p>
    ...
    ''', santri_list=santri_list, dates_week=dates_week, absen_map=absen_map, bulan_list=BULAN_LIST, bayar_map=bayar_map, current_year=current_year, today_db=today_db, materi_today=materi_today, materi_minggu_ini=materi_minggu_ini)
...
@app.route('/admin')
def admin_dashboard():
    dates_week, today_db, current_year, _ = get_minggu_sekarang()
